...
class IdentityCodec:
    """Preserve primitive values passed to test handlers."""
...
async def test_invalid_handler_payload_sends_a_failed_step_result() -> None:
    ...
    execution = Execution(
        worker,
        directive,
        RegisteredStep(handler=handler, spec=HandlerSpec(payload_parameters={"count": int})),
        ExecutionDeps(
...
async def test_multi_parameter_handler_uses_python_default_for_absent_payload_value() -> None:
    """An omitted defaulted parameter is left for Python to supply."""
    values: dict[str, int | str] = {}

    async def handler(ctx: Context, required: str, retries: int = 3) -> Directive:
    ...
    directive = Directive(
        ...
        kind=DirectiveKind.step,
        run_info=DEFAULT_RUN_INFO,
        msg=Step(step_name="current_step", payload={"required": "value"}),
    )
    worker = WorkerInfo(id=DEFAULT_WORKER_ID, name="test-worker")
    workflow_api = FakeWorkflowAPI()
    listener_factory = FakeHistoryListenerFactory()
    execution = Execution(
        ...
        RegisteredStep(
            handler=handler,
            spec=HandlerSpec(
                payload_parameters={"required": str, "retries": int},
                defaulted_payload_parameters={"retries"},
            ),
        ),
        ExecutionDeps(
            kvman=KVManager(FakeKVApi(), codec),
    ...
    assert values == {"required": "value", "retries": 3}
    assert recorder.directives[-1].kind is DirectiveKind.step_result
