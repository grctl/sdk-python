"""Spec tests: ctx.random() deterministic replay."""
...
from grctl.models.history import RandomRecorded
...
def _random_replay_worker(wf_type: str, pause_event=None) -> None:
    async def run() -> None:
...
async def test_random_returns_same_value_on_replay(grctl_client) -> None:
    pause_event = multiprocessing.Event()
    wf_type = unique_workflow_type("spec_ctx_det_random_replay")

    worker_a = multiprocessing.Process(target=_random_replay_worker, args=(wf_type, pause_event), daemon=True)
    worker_b = multiprocessing.Process(target=_random_replay_worker, args=(wf_type,), daemon=True)
    worker_a.start()
    ...
    try:
        ...
        recorded_event, _ = await history.wait_for_kind(HistoryKind.random_recorded)
        _terminate(worker_a)
        worker_b.start()

        result = await asyncio.wait_for(handle.future, timeout=30.0)

        assert result == cast("RandomRecorded", recorded_event.msg).value
    finally:
        _terminate(worker_a)
        _terminate(worker_b)
...
async def test_random_records_value_in_history(worker, grctl_client) -> None:
    wf = Workflow(workflow_type=unique_workflow_type("spec_ctx_det_random_hist"))
    ...
    try:
        ...
        history = HistoryAccess(grctl_client, wf_id, handle.run_info.id, timeout=_HISTORY_TIMEOUT)
        event, _ = await history.wait_for_kind(HistoryKind.random_recorded)

        assert 0.0 <= cast("RandomRecorded", event.msg).value < 1.0
    finally:
        await handle.future.stop()
