"""Spec tests: ctx.now() deterministic replay."""
...
from grctl.models.history import TimestampRecorded
...
def _now_replay_worker(wf_type: str, pause_event=None) -> None:
    async def run() -> None:
...
async def test_now_returns_same_value_on_replay(grctl_client) -> None:
    pause_event = multiprocessing.Event()
    wf_type = unique_workflow_type("spec_ctx_det_now_replay")

    worker_a = multiprocessing.Process(target=_now_replay_worker, args=(wf_type, pause_event), daemon=True)
    worker_b = multiprocessing.Process(target=_now_replay_worker, args=(wf_type,), daemon=True)
    worker_a.start()
    ...
    try:
        ...
        recorded_event, _ = await history.wait_for_kind(HistoryKind.timestamp_recorded)
        _terminate(worker_a)
        worker_b.start()

        result = await asyncio.wait_for(handle.future, timeout=30.0)

        assert result == cast("TimestampRecorded", recorded_event.msg).value.isoformat()
    finally:
        _terminate(worker_a)
        _terminate(worker_b)
...
async def test_now_records_timestamp_in_history(worker, grctl_client) -> None:
    ...
    history = HistoryAccess(grctl_client, wf_id, handle.run_info.id, timeout=_HISTORY_TIMEOUT)
    event, _ = await history.wait_for_kind(HistoryKind.timestamp_recorded)

    assert cast("TimestampRecorded", event.msg).value is not None
